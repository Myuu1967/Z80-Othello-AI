"""
遺伝的アルゴリズムで POS_WEIGHT テーブルを最適化する（RFCT100 評価関数版）

RFCT100.ASM の EvalLeaf と完全一致する評価式:
...
# =========================================================
# 評価関数 (RFCT100.ASM EvalLeaf と完全一致)
# =========================================================
def eval_rfct100(board, side):
    empty = board.count(oth.EMPTY)
    opp   = 3 - side

    if empty < 12:   # LATE: stone_diff * 100 + stable * 30
    ...
    if empty >= 44:  # EARLY: mob*2, stable*4
        return pos_diff + mob_diff * 2 + stable_diff * 4
    else:            # MID:   mob*2, stable*6
        return pos_diff + mob_diff * 2 + stable_diff * 6

# othello_mm3_ab の eval_board を差し替え
oth.eval_board = eval_rfct100
...
GA_D2S_PARAMS  = [114, -5, -16, -7, -54, -16, 7, 6, 2, -12]   # depth-2 signed  (2026-04-27, 現RFCT100)
...
def random_individual():
    return [max(PARAM_MIN, min(PARAM_MAX,
            v + random.randint(-40, 40))) for v in GA_D2S_PARAMS]
...
def run_ga():
    ...
    for rank, (w, p) in enumerate(ranked, 1):
        ...
        tag = ""
        if p == GA_D2S_PARAMS: tag = " [GA_D2S/現RFCT100]"
        elif p == GA_D3_PARAMS: tag = " [GA_D3]"
        elif p == V2_PARAMS:    tag = " [V2]"
    ...
    print()
    print(f"  {'名前':12s}  {'優勝':>5}  {'GA_D2S':>6}  {'GA_D3':>5}  {'V2':>5}")
    for name, val, d2s, d3, v2 in zip(
            PARAM_NAMES, best_params, GA_D2S_PARAMS, GA_D3_PARAMS, V2_PARAMS):
        diff = val - d2s
        sign = f"+{diff}" if diff >= 0 else str(diff)
        print(f"  {name:12s}: {val:5d}  (GA_D2S={d2s:4d} {sign:>4s}, GA_D3={d3:4d}, V2={v2:4d})")
