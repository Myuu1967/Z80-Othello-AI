"""
遺伝的アルゴリズムで POS_WEIGHT テーブルを最適化する（RFCT100 v2 評価関数版）

v2 の変更点: EARLY/MID フェーズに石数差ペナルティ項を追加
...
石数が多いほどスコアが下がる → 中盤に石を取り過ぎない戦略を促す
ウェイトはユーザーが play_vs_ai.py で対人試験して決定した値を採用

使い方:
  cd python
  python optimize_weights_rfct100_v2.py > ga_rfct100_v2_result.txt 2>&1
"""
...
def eval_rfct100_v2(board, side):
    ...
    if empty >= 44:  # EARLY: mob*8, stable*8, stone_diff ペナルティ*8
        return pos_diff + mob_diff * 8 + stable_diff * 8 - stone_diff * 8
    else:            # MID:   mob*8, stable*16, stone_diff ペナルティ*4
        return pos_diff + mob_diff * 8 + stable_diff * 16 - stone_diff * 4

# othello_mm3_ab の eval_board を差し替え
...
POP_SIZE      = 20
N_ELITE       = 5
N_GAMES       = 5     # GA 世代評価（粗いフィルタ）
N_GAMES_FINAL = 20    # 最終トーナメント（精密評価）
DEPTH         = 2
GENERATIONS   = 30
MUT_RATE      = 0.3
MUT_DELTA     = 15
...
GA_D2S_PARAMS       = [114, -5, -16, -7, -54, -16, 7, 6, 2, -12]    # depth-2 signed  (2026-04-27)
GA_RFCT100_PARAMS   = [157, -12, 2, 5, -49, -16, -4, -19, -15, -24] # RFCT100 優勝 (2026-04-29) ← 初期値
...
def random_individual():
    # GA_RFCT100 優勝値を起点にランダム摂動
    return [max(PARAM_MIN, min(PARAM_MAX,
            v + random.randint(-40, 40))) for v in GA_RFCT100_PARAMS]
...
def run_ga():
    ...
    print("=" * 60)
    print("GA POS_WEIGHT 最適化 (RFCT100 v2: mob×8, stable×8/16, -stone_diff×8/4)")
    print("=" * 60)
    print(f"個体数={POP_SIZE}, エリート={N_ELITE}, "
          f"GA評価={N_GAMES}ゲーム, 最終={N_GAMES_FINAL}ゲーム, "
          f"depth={DEPTH}, 世代={GENERATIONS}")
    print(f"GA_RFCT100 (初期値): {GA_RFCT100_PARAMS}")
    print(f"GA_D2S              : {GA_D2S_PARAMS}")
    print(f"GA_D3               : {GA_D3_PARAMS}")
    print()

    # 初期集団: GA_RFCT100 + GA_D2S + GA_D3 + ランダム
    population = (
    ...
    for rank, (w, p) in enumerate(ranked, 1):
        ...
        tag = ""
        if p == GA_RFCT100_PARAMS: tag = " [GA_RFCT100/現行]"
    ...
    print()
    print(f"  {'名前':12s}  {'優勝':>5}  {'RFCT100':>7}  {'GA_D2S':>6}  {'GA_D3':>5}")
    for name, val, r100, d2s, d3 in zip(
            PARAM_NAMES, best_params, GA_RFCT100_PARAMS, GA_D2S_PARAMS, GA_D3_PARAMS):
        diff = val - r100
        sign = f"+{diff}" if diff >= 0 else str(diff)
        print(f"  {name:12s}: {val:5d}  (RFCT100={r100:4d} {sign:>4s}, GA_D2S={d2s:4d}, GA_D3={d3:4d})")
