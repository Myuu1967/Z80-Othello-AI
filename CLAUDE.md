...
1. **optimize_weights.py の結果確認** — 2026-04-21夜に実行開始、翌日結果を確認してZ80テーブルに反映（次回スタート地点）
2. **D3_THRESHOLD=25 を実機計測・確定** — 30では5秒超のため25に下げて再計測
3. ~~D3_THRESHOLD を大きくして depth-3 適用範囲を拡大~~ — 30試行→5秒超、25に変更済み
4. ~~**MM2_AB_D3.ASM 終盤 depth-3 の処理時間を実機計測**~~ ✓ 完了（先手・後手ともに4秒以下確認、大会用確定）
5. **終盤完全読み（AIset_EG/SearchFull）を一時凍結** — 閾値=1 でもフリーズ発生、原因不明のため保留
6. ~~PASS連続2回・DRAW の動作テスト~~ ✓ 完了（gameDisplay.py 修正済み）
7. ~~depth-3 実装~~ ✓ 完了（MM2_AB_D3.ASM、空き<20で depth-3 切り替え）
8. ROM ブート化（オセロ完成後）
