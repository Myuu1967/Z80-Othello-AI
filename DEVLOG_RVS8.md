...
## RFCT100.ASM 実機確認・不具合発生 (2026-04-27)

### 状況

Step7完成版（再帰 negamax + mob/stable 評価）を実機確認したところ不具合が発生。
詳細は調査中。

### 対応

`git checkout 474e676 -- asm/RFCT100.ASM` で Step3（depth-1 AIset 実動版）まで戻して
動作状況を段階的に確認する方針に変更。

### 次のステップ

Step3 の動作を実機確認 → 不具合の原因を特定 → 必要な Step まで段階的に再実装。
