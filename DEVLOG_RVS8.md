...
- 主なバグ3点を修正（NEG(80H) オーバーフロー → 81H に変更、EG_BESTSCORE 退避漏れ等）
  - **NEG(80H) オーバーフロー詳細**: α の初期値（-INF）として 80H（8bit符号付き -128）を使用していたが、
    negamax では子ノードのスコアを符号反転して親に返す（`score = -SearchFull(相手)`）。
    Z80 の `NEG` 命令は `NEG(80H) = 80H`（-(-128) = 128 が 8bit に収まらずオーバーフロー）。
    結果として「相手にとって最悪の局面」を否定しても -128 のまま → α が更新されず
    α-β 枝刈りが誤って大量発火 → 探索がほとんどできない状態に。
    → **81H（-127）に変更**: `NEG(81H) = 7FH`（+127）と正しく反転するため問題なし。
...
## 第7回自作CPU大会 参加報告 (2026-05-09)

### 出場バージョン

RFCT150.ASM（モニタROM経由）で出場。RF150ROM.ASM（EPROM版）は持参したが使用せず。

### 結果

| 対戦 | 結果 | 備考 |
|---|---|---|
| vs FPGAによる自作CPU | 反則負け（時間オーバー） | 局面はほぼ優勢 |
| vs ちぇりーたくあんさん | 反則負け（時間オーバー） | 局面は相手優勢 |

**フリーズ発生**: 終盤完全読み（AIset_EG）が発動した瞬間にフリーズ。消化不良のまま終了。

決勝は上記2名の対戦となり、ちぇりーたくあんさんが優勝（自分が負けた2名が決勝進出という皮肉な展開）。

### 印象に残った展示

| 発表者 | 内容 |
|---|---|
| **ちぇりーたくあんさん** | NANDのみで構成したCPU。300KHz・2手読み。**優勝**。「評価関数が優れている」と講評。 |
| **にちかさん** | フラッシュメモリをCPUとして使用。書き込み特性（0セルに1を2回書くと1）をAND演算として解釈。セクタ単位消去で演算効率化→加算器を16倍高速化。フェイルセル対処として8セル多数決も実装。 |
| **きっちーさん** | -1/0/1の三値論理による三進コンピュータ。CMOSFETで回路を構成（論理ICなし）。3色LEDで状態表示。実動作展示。 |
| — | マインクラフト上で8bitCPUを構成。レッドストーン回路でCPU実装。クロック約1Hz。 |
| — | FPGAでCPUを構成してオセロAI。自分と対戦した相手。 |
| — | TD4基板（アリエクで安価に入手可能）を題材にした教育教材を制作中。 |

### 参加者・雰囲気

約30名。大学教授、高校生、大学院生、組み込みエンジニアなど多彩な顔ぶれ。

- BOOTHで販売中の1bitCPUを作って持参してくれた方がいた
- FPGAに詳しい方から「まずPCでエミュレータを組んで構成を理解してからFPGAに落とす」というアドバイスをもらった
- hayami1さん（教育に携わっている方）からエールをもらった。BASICで図形を描いたワクワクを共有。TD4を題材にした教育教材を制作中とのこと

### 反省・次回への展望

- フリーズ原因（AIset_EG / SearchFull）の特定・修正が最優先
- 評価関数の質が勝敗を分けることを優勝者の講評から再確認
- **次回（秋〜冬予定）に向けて自作4bitCPUの完成を目指す**（設計は完了済み）
- 自作CPUでの参加がモチベーション爆上がり

---

## ★AIset_EG フリーズ原因を特定・修正 (2026-09-03)

### 原因: `SF_OLOOP` の `ADD HL,BC` 用 B クリア漏れ（1文字違い）

`SearchFull` の `SF_OLOOP` 内、POS_ORDER_D3 のインデックス計算2箇所が誤っていた。

```asm
        LD   HL,POS_ORDER_D3
        LD   C,B
        LD   D,0        ; ← 誤り。正しくは LD B,0
        ADD  HL,BC
```

`ADD HL,BC` は B も加算に使うが、B にはループインデックスが残ったまま。
`D` のゼロクリアは無意味（直後に `LD A,(SF_SIDE_TMP) / LD D,A` で上書きされる死んだ代入）。

結果、index i は `POS_ORDER_D3[i]` ではなく **`823BH + i×257`** を読む。

このバグは `SF_HasMove` を POS_ORDER_D3 順ループに変更した際に混入。
RFCT120（完全読み未使用）には存在せず、RFCT150 / RF150ROM のみに存在した。

### HEXイメージによる実挙動の再現

`RFCT150.hex`（収録範囲 8000H-94C8H）から読み出しアドレスを再現した結果:

| 項目 | 結果 |
|---|---|
| 正しく読める index | **i=0 のみ**（偶然 `POS_ORDER_D3[0]=00`=A1） |
| プログラム領域内を読む | 16/64件（コードのバイト列をマス番号として解釈） |
| **プログラム範囲外 954EH〜C17AH** | **48/64件 → 電源投入時の不定RAM** |
| 走査される異なるマス | **11マスのみ**（本来64マス全て）／A1が4回重複 |

row/col は `AND 07H` でマスクされるため盤面外アクセスにはならず、
「異常終了せず固まる」挙動になっていた。

### フリーズのメカニズムと、症状すべての説明

1. **断続性** — 64件中48件が不定RAM読み出しのため、探索内容が電源投入ごとに変化する。
   ラボで再現せず大会本番で発生した理由がこれ。
2. **停止** — 不定RAMに同一の合法手を指す値が多数並ぶと、同じ手を各ノードで何十回も
   重複探索する。分岐数 k に対し k^6（空き4 → 最大6手先）に膨張し、10MHz では事実上停止。
3. **空き=2 のテスト（2026-05-06 2回目）が正常終了した理由** — 深さ2程度では k² にしか
   ならず間に合ってしまう。空き4で初めて破綻する。
4. **「局面はほぼ優勢」だったのに負けた理由** — 探索が破綻しており完全読みのスコアが不正。
   合法手が1件も見つからないと `EG_BESTSCORE` が sentinel `81H`(-127) のまま返り、
   `NEG` されて +127 という無意味な値が伝播する。

2026-05-06 のコード精査で「定義的なバグは発見されず」としたのは、
`SaveBoard`/`HasAnyLegalMove`/スタック均衡/α-β を重点的に見ており、
インデックス計算のレジスタ指定を見落としていたため。

### 修正内容

`RFCT150.ASM`（2326行・2351行）、`RF150ROM.ASM`（2276行・2301行）:

```asm
-        LD   D,0
+        LD   B,0                ; ★修正: ADD HL,BC 用に B をクリア
```

B はこの直後に row で上書きされ、ループ末尾の `POP BC` で復元されるため副作用なし。

### 横展開チェック

`RFCT150.ASM` 内の `ADD HL,BC` 全24箇所について B のクリア漏れを点検。
`NM_GetSaveBuf`・`NM_RECURSE` を含め他はすべて正しく `LD B,0` されており、
同種のバグは SF_OLOOP の2箇所のみだった。
...
| `python/measure_eg_nodes.py` | commit 784f92b で追加（作業中）。終盤完全読みの正当なノード数を Z80 と同手順（POS_ORDER_D3 順・alpha[depth] 方式・PASSはdepth消費）で実測するスクリプト |
...
**終盤でフリーズしないオセロであることを実機で確認すること。**
機能追加・性能向上（評価関数の質、`ENDGAME_THRESHOLD` 引き上げ、`SF_NODE_CAP` 実装）は
すべてその後に回す。CLAUDE.md「現在の状況」の冒頭とプロジェクトメモリに明記した。

### フォールバック版の特定

RFCT150 が実機で不安定なときの差し替え先は **`asm/RFCT120.ASM`**。

- DEVLOG 2026-05-03「RFCT120 実機確認・大会出場候補確定」で昇格した版。処理時間4秒以下を実機確認済み
- RFCT150 は RFCT120 のコピーに完全読みを足したものなので、フリーズ箇所 `SF_OLOOP` が存在しない（grep 0件）
- `ENDGAME_THRESHOLD EQU 0` により `CALL AIset_EG` の到達条件が「空き ≤ 0」となり、完全読みは事実上呼ばれない
- **RFCT120 には ROM（27C256）起動版がない**。EPROM 単独起動でフォールバックするなら別途作成が必要。
  モニタ ROM 経由なら `RFCT120.hex`（2026-05-03 ビルド）をそのままロードできる

`asm/RFCT120_CP.ASM` を作業用コピーとして追加（2026-09-08、アセンブル・動作確認とも未実施）。
原本との差分は `LINE_MAX EQU 20` へのコメント追記1行のみ。

### ドキュメント体系の再編

読み込みの入口を一本化し、詳細を分割した。

| ファイル | 役割 |
|---|---|
| `CLAUDE.md`（425行 → 102行） | **入口**。ドキュメント全体構成の表、現在の状況、作業対象、開発フロー、ハード仕様、Git規約 |
| `docs/PITFALLS.md` | 既知の注意事項（アセンブラを書く前に必ず読む） |
| `docs/ARCHITECTURE.md` | 版の系譜・実装済み機能・評価式・POS_WEIGHT・実測値・ROM化構成・フォールバック版 |
| `docs/REFERENCE.md` | 関数リファレンス・RFCT000 命名規則 |
| `docs/TODO.md` | TODO 一覧（消化済み含む） |

分割で文章が欠落していないことは全行照合で検証済み（差分2行はいずれも意図的な更新）。

あわせて公開／非公開の切り分けを決めた。公開リポジトリ（README.md・DEVLOG・CLAUDE.md・docs/）には
対外的に意味のある情報のみを置き、細かい内部メモは `README.local.md`（`.gitignore` の `*.local.md` で
除外）に書く。

### 次回作業の手順

1. `RFCT150.ASM` をアセンブル（`.hex` は 2026-09-03 生成済み・Errors 0 なので流用可）
2. 空きマス4の局面まで対局を進め、完全読み発動でフリーズしないことを確認
3. Pico GPIO15 で処理時間を計測
4. 結果を DEVLOG に追記（フリーズが再発したら `RFCT120` に退避）
