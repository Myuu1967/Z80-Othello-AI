"""
遺伝的アルゴリズムで POS_WEIGHT テーブルを最適化する（depth-3 × v2 評価関数版）

評価関数は optimize_weights_rfct100_v2.py と同一:
  EARLY (empty >= 44): pos_diff + mob_diff*8 + stable_diff*8  - stone_diff*8
  MID   (empty >= 12): pos_diff + mob_diff*8 + stable_diff*16 - stone_diff*4
  LATE  (empty <  12): stone_diff*100 + stable_diff*30

depth-2 版 (v2) との違い:
  DEPTH = 3  → EvalLeaf は 3 手後の局面を評価
  depth-3 ではコーナー重みが低め・内陸負値が緩めになる傾向

初期個体の基点: GA_D3_PARAMS (depth-3 で過去に最適化された値)
                GA_D2S_PARAMS (depth-2 優勝値も競合として投入)

使い方:
  cd python
  python optimize_weights_d3_v2eval.py > result_d3_v2eval.txt 2>&1
"""
...
# =========================================================
# 評価関数 (v2: stone_diff ペナルティ付き、v2 スクリプトと同一)
# =========================================================
def eval_rfct100_v2(board, side):
    ...
    my_mob  = oth.count_mobility(board, side)
    opp_mob = oth.count_mobility(board, opp)
...
N_GAMES_FINAL = 10    # 最終トーナメント（精密評価）
DEPTH         = 3
GENERATIONS   = 20
MUT_RATE      = 0.3
MUT_DELTA     = 15
...
GA_D3_PARAMS      = [66, -32, -1, 44, -45, 22, 1, 10, 23, -29]    # depth-3 最適化 (旧eval, 2026-04-26)
GA_D2S_PARAMS     = [114, -5, -16, -7, -54, -16, 7, 6, 2, -12]    # depth-2 優勝 (v2 eval, 2026-04-29)
GA_RFCT100_PARAMS = [157, -12, 2, 5, -49, -16, -4, -19, -15, -24] # depth-2 旧優勝 (rfct100 eval)
...
def random_individual():
    # GA_D3_PARAMS を起点にランダム摂動（depth-3 の既知良好値）
    return [max(PARAM_MIN, min(PARAM_MAX,
            v + random.randint(-40, 40))) for v in GA_D3_PARAMS]
...
def run_ga():
    ...
    print("=" * 60)
    print("GA POS_WEIGHT 最適化 (depth-3 × v2eval: mob×8, stable×8/16, -stone×8/4)")
    print("=" * 60)
    print(f"個体数={POP_SIZE}, エリート={N_ELITE}, "
          f"GA評価={N_GAMES}ゲーム, 最終={N_GAMES_FINAL}ゲーム, "
          f"depth={DEPTH}, 世代={GENERATIONS}")
    print(f"GA_D3     (初期値/ベースライン): {GA_D3_PARAMS}")
    print(f"GA_D2S    (depth-2 参考値)     : {GA_D2S_PARAMS}")
    print(f"GA_RFCT100(depth-2 旧優勝)     : {GA_RFCT100_PARAMS}")
    print()

    # 初期集団: GA_D3 + GA_D2S + GA_RFCT100 + ランダム
    population = (
        [GA_D3_PARAMS[:], GA_D2S_PARAMS[:], GA_RFCT100_PARAMS[:]]
        + [random_individual() for _ in range(POP_SIZE - 3)]
    )

    best_ever_score  = (-1, -999999)
    best_ever_params = None
    baseline_params  = GA_D3_PARAMS[:]   # depth-3 の既知値をベースラインに
    hall_of_fame     = [GA_D3_PARAMS[:], GA_D2S_PARAMS[:], GA_RFCT100_PARAMS[:]]
    t_start = time.time()
    ...
    print("=" * 60)
    print(f"精密トーナメント ({N_GAMES_FINAL}ゲーム/ペア, depth={DEPTH})")
    print(f"候補数: {len(hall_of_fame)}")
    print()
    t_tourn = time.time()
    win_counts = tournament(hall_of_fame, n_games=N_GAMES_FINAL)
    print(f"トーナメント完了  所要時間: {(time.time()-t_tourn)/60:.1f}分")
    print()
    ...
    for rank, (w, p) in enumerate(ranked, 1):
        ...
        tag = ""
        if p == GA_D3_PARAMS:      tag = " [GA_D3/ベースライン]"
        elif p == GA_D2S_PARAMS:   tag = " [GA_D2S]"
        elif p == GA_RFCT100_PARAMS: tag = " [GA_RFCT100]"
    ...
    print()
    print(f"  {'名前':12s}  {'優勝':>5}  {'GA_D3':>5}  {'GA_D2S':>6}  {'RFCT100':>7}")
    for name, val, d3, d2s, r100 in zip(
            PARAM_NAMES, best_params, GA_D3_PARAMS, GA_D2S_PARAMS, GA_RFCT100_PARAMS):
        diff = val - d3
        sign = f"+{diff}" if diff >= 0 else str(diff)
        print(f"  {name:12s}: {val:5d}  (GA_D3={d3:4d} {sign:>4s}, GA_D2S={d2s:4d}, RFCT100={r100:4d})")
