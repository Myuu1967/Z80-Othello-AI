...
def run_ga():
    ...
    print(f"個体数={POP_SIZE}, エリート={N_ELITE}, "
          f"評価ゲーム={N_GAMES}, depth={DEPTH}, 世代={GENERATIONS}")
    ...
    for gen in range(GENERATIONS):
        ...
        # 全個体を評価（基準: V1）
        scored = [(evaluate(ind, V1_PARAMS), ind) for ind in population]
    ...
    print()
    print("=" * 60)
    print(f"完了  合計時間: {total_time/60:.1f}分")
    print(f"最良スコア: {best_ever_score}/{N_GAMES}勝 (V1基準)")
    print(f"最良パラメータ: {best_ever_params}")
    print()
    print("--- Z80 DEFB テーブル ---")
    print_table(best_ever_params)
    print()
    print("--- 各マス分類の値 ---")
    for name, val, v1 in zip(PARAM_NAMES, best_ever_params, V1_PARAMS):
        diff = val - v1
        sign = f"+{diff}" if diff >= 0 else str(diff)
        print(f"  {name:12s}: {val:3d}  (V1={v1:3d}, {sign})")

    return best_ever_params
